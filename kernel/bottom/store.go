...
func (s *Store) initAndLoad(confPath string, filePath string, meta *meta.Meta, logs *raft_log.RaftLogSet,
	m Medium,
...
func (s *Store) appendLogs(logs *[]raft_log.RaftLog) error {
	for _, v := range *logs {
		if err := s.medium.Append(s.filePath, raft_log.LogToString(v)+"\n"); err != nil {
			log.Println(err)
		}
	}
	return nil
...
func (s *Store) loadFrom0(logPath string, logs *raft_log.RaftLogSet) error {
	var str string
